-- Persist Defense signal as a VIEW (MVP)
-- View name: signal.v_defense_s3a

CREATE SCHEMA IF NOT EXISTS signal;

CREATE OR REPLACE VIEW signal.s3a_defense_intensity_v1 AS
WITH
params AS (
  SELECT
    'semis'::text     AS policy_domain,
    'DOD'::text       AS agency_key,
...
-- 2) match to whitelist phrases (DOD)
agency_hits AS (
  SELECT
    ae.filing_uuid,
    ae.activity_idx,
...
scored AS (
  SELECT
    h.anchor_id,
    h.window_id,
    'S3a_defense_intensity'::text AS signal_type,
